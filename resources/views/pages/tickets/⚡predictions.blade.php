...
new #[Title('Submit Predictions')] class extends Component {
    ...
    public function submit(): void
    {
        ...
        foreach ($matches as $match) {
            if ($this->isScoreMode) {
                $predictionsData[$match->id] = [
                    'team_1_score' => (int) $this->scores1[$match->id],
                    'team_2_score' => (int) $this->scores2[$match->id],
                ];
            } else {
                $predictionsData[$match->id] = [
                    'predicted_result' => $this->predictions[$match->id],
                ];
            }
        }
        ...
        (new SubmitPredictions)->execute($user, $ticket, $predictionsData);
